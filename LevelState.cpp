...
namespace	// Global to this file only
{
	...
	BaseObject* CreateMouseCursor() {
		BaseObject* tempCurs = new BaseObject();

		ViewManager::GetInstance().AddObject(tempCurs, 5);
		tempCurs->SetTexture("Resources/images/reticle.png");
		tempCurs->SetDimensions(32, 32);
		tempCurs->SetColor(0.74f, 1, 1, 1);
	...
	ProgressBar* CreateProgressBar() {
		...
		tempBar->SetTexture("Resources/images/progressBar2.png");
		tempBar->SetDimensions(32.0f, 8.0f);

		return tempBar;
	}

	//Ship* CreatePlayerShip() {
	//	Ship* tempShip = SaiphApp::CloneShipFromPrototype(SaiphApp::GetShipID());

	//	tempShip->SetShield(SaiphApp::GetShieldFlyweight(SaiphApp::GetShieldID()).strength);
	//	tempShip->SetBattery(SaiphApp::GetGeneratorFlyweight(SaiphApp::GetGeneratorID()).battery);
	//	tempShip->SetPosition(BOUNDS_SIZE / 2.0f, BOUNDS_SIZE / 2.0f);
	//	tempShip->SetAfterburner(SaiphApp::GetPropulsionFlyweight(SaiphApp::GetPropulsionID()).afterburnerEnergy);
	//
	//	return tempShip;
	//}
	...
	BaseObject* CreateBackground() {
		...
		background->SetPosition(BOUNDS_SIZE / 2.0f, BOUNDS_SIZE / 2.0f);
		background->SetDimensions(BOUNDS_SIZE * 1.5f, BOUNDS_SIZE);

		return background;
	}

	PowerUp* CreatePowerUp(const Enemy* enemy, const Vec2f velocity) {
		PowerUp* tempPower = new PowerUp();
		ViewManager::GetInstance().AddObject(tempPower, 2);
		tempPower->SetVelocity(velocity);
		tempPower->SetType(enemy->GetPowerUpID());

		switch (tempPower->GetType()) {
		case 0:
			tempPower->SetTexture("Resources/images/pupscore.png");
			break;
		case 1:
			tempPower->SetTexture("Resources/images/pupenergy.png");
			break;
		case 2:
			tempPower->SetTexture("Resources/images/puparmor.png");
			break;
		case 3:
			tempPower->SetTexture("Resources/images/pupweapon.png");
			break;
		default:
			break;
		}

		tempPower->SetDimensions(32.0f, 32.0f);
		tempPower->SetPosition(enemy->GetShip()->GetPosition());
	...
	Ship* CreatePlayerShip() {
		Ship* toDelete = SaiphApp::CloneShipFromPrototype(SaiphApp::GetShipID());

		MotherShip* ship = new MotherShip(*toDelete);
		ship->SetShield(SaiphApp::GetShieldFlyweight(SaiphApp::GetShieldID()).strength);
		ship->SetBattery(SaiphApp::GetGeneratorFlyweight(SaiphApp::GetGeneratorID()).battery);
		ship->SetPosition(BOUNDS_SIZE / 2.0f, BOUNDS_SIZE / 2.0f);
		ship->SetAfterburner(SaiphApp::GetPropulsionFlyweight(SaiphApp::GetPropulsionID()).afterburnerEnergy);

		for (int i = 0; i < 5; i++) {
			Ship* temp = SaiphApp::CloneShipFromPrototype(SaiphApp::GetShipID(), 1);
			temp->SetColor(0.9f, 0.8f, 0.7f, 0.6f);

			ship->add_shadow(temp);
		}

		ViewManager::GetInstance().RemoveObject(toDelete);
		delete toDelete;
		ViewManager::GetInstance().AddObject(ship, 2);
...
void LevelState::GeneratePowerUps(Enemy *_enemy)
{
	...
	for(unsigned int i = 0; i < _enemy->GetPowerUpAmount(); ++i)
	{
		...
		powerUps.push_back(CreatePowerUp(_enemy, randomVelocity));
	}
}
...
unsigned int LevelState::CheckForInput()
{
	...
	if(!gamePaused)
	{
		// If 'E' is pressed - toggle deflector
		if (GetAsyncKeyState('E') & 0x1) {
			if (!playerShip->GetDeflector()) {	// if deflector is nullptr
				Deflector* deflector = new Deflector(playerShip, this);
				playerShip->SetDeflector(deflector);
				playerShip->GetDeflector()->SetActive(true);
			}
			else {
				playerShip->GetDeflector()->SetActive(!(playerShip->GetDeflector()->GetActive()));
			}
		}

		if ((GetAsyncKeyState('F') & 0x1) && playerShip->GetDeflector()) {
			playerShip->GetDeflector()->ActivateStrategy("FStrategy");
		}
		if ((GetAsyncKeyState('G') & 0x1) && playerShip->GetDeflector()) {
			playerShip->GetDeflector()->ActivateStrategy("GStrategy");
		}

		// Ship movement
		if(playerShip)
		{
			// If 'C' is pressed - fire sticky bombs
			if (GetAsyncKeyState('C') & 0x1) {
				if (playerShip->GetBattery() > 10) {
					for each (Enemy * enemy in enemies) {
						if (enemy->GetShip()->Collide(*cursor)) {
							playerShip->SetBattery(playerShip->GetBattery() - 10);

							StickyBomb* stickyBomb = new StickyBomb();
							stickyBomb->SetTarget(enemy);
							enemy->Attach(stickyBomb);

							// fire stickyBomb
							ViewManager::GetInstance().AddObject(stickyBomb, 1);
							stickyBomb->SetPosition(playerShip->GetPosition());
							stickyBomb->SetHeading(stickyBomb->GetHeading() + playerShip->GetHeading());
							playerBullets.push_back(stickyBomb);

							stickyBomb->Heartbeat(0.01f);
							stickyBomb->CollisionResponse();
						}
					}
				}
			}

			// If 'X' is pressed - explode sticky bombs
			if (GetAsyncKeyState('X') & 0x1) {
				for each (Enemy * enemy in enemies) {
					enemy->NotifyObservers(ObserverEvent::ACTIVATE);
				}
			}
